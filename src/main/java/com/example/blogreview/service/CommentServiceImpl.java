...
public class CommentServiceImpl implements CommentService {
    ...
    public CommentResponseDto createComment(Long postId, CommentRequestDto requestDto, UserDetailsImpl userDetails) {
        ...
        Comment comment = commentRepository.save(new Comment(requestDto, userDetails.getUser(), post));
        return new CommentResponseDto(comment);
    }

    @Override
    @Transactional
    public ResponseEntity<String> updateCommnet(Long id, Long commentid, CommentRequestDto requestDto, UserDetailsImpl userDetails, HttpServletResponse res) {
        // 해당 게시글이 있는지 확인
        findPost(id);
        // 해당 댓글이 있는지 확인
        ...
        if (comment.getUser().getUsername().equals(userDetails.getUsername())
                || userDetails.getUser().getRole().getAuthority().equals("ROLE_ADMIN")) {
            // 있으면 댓글 내용 업데이트
    ...
    @Override
    public ResponseEntity<String> deleteComment(Long id, Long commentid, UserDetailsImpl userDetails) {
        // 해당 게시글이 있는지 확인
        findPost(id);
        // 해당 댓글이 있는지 확인
        Comment comment = findComment(commentid);
